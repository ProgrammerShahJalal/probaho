...
const Details: React.FC<Props> = (props: Props) => {
    ...
    const renderFieldValue = (fieldName: string) => {
        switch (fieldName) {
            ...
            case 'image':
                return state.item[fieldName] ? (
                    <span style={{ color: 'white' }}>Image available (see below)</span>
                ) : (
                    <span style={{ color: '#9ca3af', fontStyle: 'italic' }}>No image provided</span>
                );
            default:
                        ...
                        <div className="content_body custom_scroll">
                            ...
                            {state.item.image && (
                                <div className="details_section mt-4">
                                    <h5 className="details_section_title mb-3">Image</h5>
                                    <div className="mt-2" style={{ maxWidth: '60%', margin: 'auto' }}>
                                        
                                            <img
                                                src={state.item.image.startsWith('http') 
                                                    ? state.item.image 
                                                    : `/${state.item.image}`}
                                                alt="Branch Class Building Image"
                                                style={{ 
                                                    width: '100%', 
                                                    height: 'auto', 
                                            ...
                                            />
                                        <a
                                            href={state.item.image.startsWith('http') 
                                                ? state.item.image
                                                : `/${state.item.image}`}
                                            target="_blank"
