...
async function all(
    ...
    if (query_param.select_fields) {
        ...
        select_fields = ['id', 'title', 'date', 'description', 'file', 'status', 'created_at', 'updated_at', 'deleted_at'];
    }
    ...
    if (search_key) {
        ...
        const academic_rules_types_ids = await models.AcademicRulesTypesModel.findAll({
            where: { title: { [Op.like]: `%${search_key}%` } },
            attributes: ['id'],
        ...
        addJsonSearchConditions('academic_rules_types_id', academic_rules_types_ids);
    ...
    try {
        ...
        const afterFindHook = async (results: any[], options: AfterFindHookOptions) => {
            ...
            const academic_rules_types_ids = new Set<number>();
            ...
            results.forEach((item) => {
                ...
                if (item.academic_rules_types_id) {
                    item.academic_rules_types_id.forEach((id: number) => academic_rules_types_ids.add(id));
                }
            });
        
            // Fetch all related data in bulk
            const [users, branches, academic_years, academic_rules_types] = await Promise.all([
                ...
                models.AcademicRulesTypesModel.findAll({
                    where: { id: { [Op.in]: [...academic_rules_types_ids] } },
                    attributes: ['id', 'title'],
                }) as Promise<AcademicRulesType[]>,
            ]);
            ...
            const academic_rules_types_map = new Map(academic_rules_types.map((rules_type) => [rules_type.id, rules_type]));
            ...
            for (const item of results) {
                ...
                if (item.academic_rules_types_id) {
                    item.dataValues.academic_rules_types = item.academic_rules_types_id
                        .map((id: number) => academic_rules_types_map.get(id))
                        .filter(Boolean);
                }
            }
        };
