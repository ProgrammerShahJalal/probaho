...
export default async function import_academic_years(
    ...
    try {
        ...
        const fileContent = (await file.toBuffer()).toString('utf8');
        ...
        return new Promise((resolve, reject) => {
            Papa.parse<AcademicYearImportData>(fileContent, {
                ...
                complete: async (results) => {
                    ...
                    for (const [index, row] of results.data.entries()) {
                        ...
                        try {
                            if (
                                ...
                                throw new Error(
                                    `Missing required fields (title, start_month, end_month).`,
                                );
                            }
                            ...
                            errors.push(
                                `CSV Row ${csvRowNumber}: ${error.message}`,
                            );
                            failedImports++;
                        }
                    }
                    ...
                    resolve(
                        response(
                            errors.length > 0 && successfulImports === 0
                                ? 400
                                : 200,
                            `Import process completed. Successful: ${successfulImports}, Failed: ${failedImports}.`,
                            {
        ...
        return response(
            error.status || 500,
            error.message ||
                'An unexpected error occurred during academic year import.',
            {},
        );
    }
}
