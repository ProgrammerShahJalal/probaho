...
async function all(
    ...
    if (query_param.select_fields) {
        ...
        select_fields = ['id', 'branch_user_id', 'branch_id', 'academic_year_id', 'branch_class_building_id', 'title', 'code', 'capacity', 'status', 'created_at', 'updated_at', 'deleted_at'];
    }
    ...
    if (search_key) {
        ...
        const [user_ids, branch_ids, academic_year_ids, branch_class_building_ids] = await Promise.all([
            ...
            fetchRelatedIds(models.BranchClassBuildingsModel, 'title'),
        ]);
        ...
        addJsonSearchConditions('branch_class_building_id', branch_class_building_ids);
    ...
    try {
        ...
        const afterFindHook = async (results: any[], options: AfterFindHookOptions) => {
            ...
            const branch_class_building_ids = collectIds('branch_class_building_id');
        
            // Fetch all related data in bulk
            const [users, branches, academic_years, branch_class_buildings] = await Promise.all([
                ...
                models.BranchClassBuildingsModel.findAll({
                    where: { id: { [Op.in]: [...branch_class_building_ids] } },
                    attributes: ['id', 'title'],
                }) as Promise<BranchClassBuilding[]>,
            ]);
            ...
            const branch_class_building_map = new Map(branch_class_buildings.map((building: BranchClassBuilding) => [building.id, building]));
            ...
            for (const item of results) {
                ...
                attachRelatedData(item, 'branch_class_building_id', 'branch_class_buildings', branch_class_building_map);
            }
        };
